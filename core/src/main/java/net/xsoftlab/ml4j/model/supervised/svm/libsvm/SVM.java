...
public class SVM {
	...
	private static void solve_c_svc(Problem prob, Parameter param, double[] alpha, Solver.SolutionInfo si,
			double Cp, double Cn) {
		int l = prob.length;
	...
	private static void svm_binary_svc_probability(Problem prob, Parameter param, double Cp, double Cn,
		...
		for (i = 0; i < nr_fold; i++) {
			...
			subprob.x = new Node[subprob.length][];
	...
	private static void svm_group_classes(Problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret,
		...
		if (param.svm_type == Parameter.ONE_CLASS || param.svm_type == Parameter.EPSILON_SVR
			...
			for (i = 0; i < nr_class; i++)
				for (int j = i + 1; j < nr_class; j++) {
					...
					sub_prob.x = new Node[sub_prob.length][];
	...
	public static void svm_cross_validation(Problem prob, Parameter param, int nr_fold, double[] target) {
		...
		if ((param.svm_type == Parameter.C_SVC || param.svm_type == Parameter.NU_SVC) && nr_fold < l) {
			int[] tmp_nr_class = new int[1];
			int[][] tmp_label = new int[1][];
			int[][] tmp_start = new int[1][];
			int[][] tmp_count = new int[1][];
		...
		for (i = 0; i < nr_fold; i++) {
			...
			if (param.probability == 1
					&& (param.svm_type == Parameter.C_SVC || param.svm_type == Parameter.NU_SVC)) {
	...
	public static double svm_predict_values(Model model, Node[] x, double[] dec_values) {
		int i;
		if (model.param.svm_type == Parameter.ONE_CLASS || model.param.svm_type == Parameter.EPSILON_SVR
	...
	public static String svm_check_parameter(Problem prob, Parameter param) {
		...
		if (kernel_type != Parameter.LINEAR && kernel_type != Parameter.POLY
				&& kernel_type != Parameter.RBF && kernel_type != Parameter.SIGMOID
		...
		if (svm_type == Parameter.C_SVC || svm_type == Parameter.EPSILON_SVR
	...
	public static int svm_check_probability_model(Model model) {
		if (((model.param.svm_type == Parameter.C_SVC || model.param.svm_type == Parameter.NU_SVC)
				&& model.probA != null && model.probB != null)
				|| ((model.param.svm_type == Parameter.EPSILON_SVR || model.param.svm_type == Parameter.NU_SVR) && model.probA != null))
			return 1;
		else
			return 0;
	}
